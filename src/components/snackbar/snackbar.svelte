<script lang="ts">
	...
	const bgColors = {
		error: 'bg-red-200/90',
		info: 'bg-blue-200/90',
		success: 'bg-green-200/90',
		warning: 'bg-yellow-200/90'
	};
	const textColors = {
		error: 'text-red-800',
		info: 'text-blue-800',
...
{#if snackbarStore.show}
	<div
		class={`fixed top-0 left-1/2 z-80 mt-6 flex max-w-[90vw] min-w-[280px] -translate-x-1/2 items-start rounded-sm px-6 py-4
				${bgColors[snackbarStore.type] ?? 'bg-gray-100/80'}
				border border-${bgColors[snackbarStore.type]} shadow-2xl backdrop-blur-sm
			`}
	>
