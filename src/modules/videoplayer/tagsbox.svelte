...
{#if isBoxOpen}
	<div class="rounded-md bg-gray-800 p-2 shadow-md">
		<div class="flex flex-wrap gap-2">
			{#if timeline.eventsPlaying.length > 0}
				{#if timeline.eventsPlaying[timeline.eventsPlaying.length - 1].tagsRelated.length === 0}
					<p class="text-sm text-gray-400">No tags related to the current play.</p>
				{:else}
					{#each timeline.eventsPlaying[timeline.eventsPlaying.length - 1].tagsRelated || [] as tag (tag)}
